...
class Blockchain:
    ...
    def get_previous_block(self):
        return self.chain[-1]  # genesis block이 있다는 전제임
    ...
    def replace_chain(self) -> Boolean:
        network = self.nodes
        longest_chain = None
        max_length = len(self.chain)
        for node in network:
            response = requests.get(f'{node}/get_chain')
            if response.status_code == 200 and response.json()['length'] > max_length and self.is_chain_valid(response.json()['chain']):
